...
class TranscriptResolver:
    ...
    def _query_vep_for_variant(self, variant: Dict) -> Optional[str]:
        """Query VEP with a variant dictionary and extract the best transcript."""
        ...
        if not all([chromosome, position, reference, alternate, gene]):
            return None
        
        # Explicitly satisfy the linter that gene is a string now
    ...
    def _query_vep_genomic_coordinates(self, chromosome: str, position: int, 
                                      reference: str, alternate: str, 
                                      gene: str, protein_change: str) -> Optional[str]:
        """Query VEP API using genomic coordinates with POST for reliability."""
        variant = {
            "chromosome": chromosome,
            "position": position,
            "reference": reference,
        ...
        return self._query_vep_for_variant(variant)

    def _query_vep_hgvs(self, hgvs: str, gene: str, protein_change: str) -> Optional[str]:
        """Query VEP API using HGVS notation"""
        
        try:
            logger.info(f"Querying VEP with HGVS: {hgvs}")
            
            url = f"{self.vep_api_base}/homo_sapiens/hgvs/{quote(hgvs)}"
            data = self._make_api_request(url)
            ...
            logger.warning(f"Error querying VEP with HGVS: {e}")
    ...
    def _get_all_vep_transcripts(self, variant: Dict) -> List[str]:
        """Fetch all transcript IDs from VEP for a given variant using POST endpoint"""
        try:
            chromosome = variant.get('chromosome')
            position = variant.get('position')
            reference = variant.get('reference')
            ...
            for entry in data:
                if 'transcript_consequences' in entry:
                    for t in entry['transcript_consequences']:
                        if t.get('transcript_id'):
                            transcript_ids.add(t['transcript_id'])
            return sorted(list(transcript_ids))
        except Exception as e:
            logger.warning(f"Error fetching all VEP transcript IDs: {e}")
            return []
